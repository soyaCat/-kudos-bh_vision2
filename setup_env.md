...
sudo apt-get update
  ...
  #### 15.도커에 usb포트에 관한 권한을 부여한다. 찾느라 고생했으니까 꼭 좀 해줬으면 한다. 명령어를 입력했으면 도커를 커밋해서 상태를 저장하고 컨테이너를 종료했다가 다시 실행시킨다. 시리얼 포트 테스트를 위해 아두이노 IDE를 설치하고 간단한 코드 업로드가 되는지 확인하고 되면 
  ```
